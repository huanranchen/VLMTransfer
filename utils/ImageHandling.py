...
def get_image(path: str = "image.jpg", return_tensor=True) -> Tensor:
    image = Image.open(path)
    image = image.convert("RGB")
    transform = transforms.ToTensor()
    return transform(image) if return_tensor else image


def get_list_image(path: str, sort_key=lambda x: int(x[:-4]), return_tensor=True, ndim=4) -> List[Tensor]:
    result = []
    images = os.listdir(path)
    images = [i for i in images if i.endswith((".jpg", ".png", ".jpeg"))]
    images.sort(key=sort_key)
    for image in images:
        cur_image = get_image(os.path.join(path, image), return_tensor=return_tensor)
        result.append(cur_image.unsqueeze(0) if ndim == 4 else cur_image)
    return result
...
def concatenate_image(
    ...
    if imgs is None:
        ...
        alls = [(img * 255).cpu().squeeze().permute(1, 2, 0).numpy() for img in imgs]
    img_shape = alls[0].shape[-3:]
    alls = [np.pad(img, padding_config) for img in alls]
    alls = alls[: row * col]
    x = np.stack(alls)
